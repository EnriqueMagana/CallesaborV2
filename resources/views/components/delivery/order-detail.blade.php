...
<div class="delivery-modal-layer" role="presentation" wire:key="delivery-detail-{{ $order->id }}" x-data
    x-on:keydown.escape.window="$wire.closeOrder()" x-init="$nextTick(() => $refs.closeButton.focus())">
    <button type="button" class="delivery-modal-layer__backdrop" wire:click="closeOrder"
        aria-label="Cerrar detalle"></button>
    <section class="delivery-detail" role="dialog" aria-modal="true" aria-labelledby="delivery-detail-title"
        aria-describedby="delivery-detail-description" tabindex="-1">
        ...
        <div class="delivery-detail__body">
                ...
                <div>
                    ...
                    <div class="delivery-detail__quick-actions">
                        @if ($order->customer_phone)
                            <a class="delivery-mini-action"
                                href="tel:{{ preg_replace('/\D+/', '', $order->customer_phone) }}"><i
                                    class="bx bx-phone"></i> Llamar</a>
                        @endif
                    </div>
                </div>
            </section>
            ...
            <section class="delivery-detail__items" aria-labelledby="delivery-products-title">
                <div class="delivery-detail__section-title">
                    <div><span><i class="bx bx-shopping-bag"></i></span>
                        <h3 id="delivery-products-title">Productos</h3>
                    </div>
                ...
                @foreach ($order->items->where('is_cancelled', false) as $item)
                    <article class="delivery-product">
                        <div class="delivery-product__heading">
                            ...
                            <div><strong>{{ $item->product_name }}</strong><small>${{ number_format($item->product_price, 2) }}
                                    c/u</small></div>
                            <b>${{ number_format($item->subtotal, 2) }}</b>
                        </div>
                        @if ($item->addons->isNotEmpty())
                            <div class="delivery-product__options"><small>Complementos</small>
                                <p>{{ $item->addons->map(fn($addon) => $addon->addon_name . ' ×' . $addon->quantity)->implode(' · ') }}
                                </p>
                            </div>
                        @endif
                        @if ($item->ingredients->isNotEmpty())
                            <div class="delivery-product__options"><small>Ingredientes</small>
                                <p>{{ $item->ingredients->map(fn($ingredient) => $ingredient->ingredient_name . ' ×' . $ingredient->quantity)->implode(' · ') }}
                                </p>
                            </div>
                        @endif
                        @if ($item->notes)
                            <div class="delivery-product__note"><i
                                    class="bx bx-note"></i><span>{{ $item->notes }}</span></div>
                        @endif
                    </article>
                @endforeach
            </section>
            ...
            @if ($order->notes)
                <section class="delivery-detail__notes"><i class="bx bx-message-square-detail"></i>
                    <div><small>Nota general</small>
                        <p>{{ $order->notes }}</p>
                    </div>
                </section>
            @endif
        ...
        <footer class="delivery-detail__footer">
            ...
            @if ($canFinish)
                <button type="button" class="delivery-btn delivery-btn--success"
                    wire:click="askToMarkDelivered({{ $order->id }})"><i class="bx bx-check-double"></i> Marcar
                    entregado</button>
            @endif
        </footer>
    </section>
