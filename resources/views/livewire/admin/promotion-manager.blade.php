                ...
                @forelse($this->promotions as $promotion)
                    ...
                    <article class="promotion-card is-{{ $promotion->presentation_type }}" wire:key="promotion-{{ $promotion->id }}">
                        ...
                        <div class="promotion-card__body">
                            ...
                            <div class="promotion-card__channels">@if($promotion->show_on_pos)<span><i class="bx bx-store-alt"></i>POS</span>@endif @if($promotion->show_on_kiosk && !$promotion->isProductLaunch())<span><i class="bx bx-devices"></i>Kiosco</span>@endif @if($promotion->show_on_digital_menu)<span><i class="bx bx-mobile-alt"></i>Menú digital</span>@endif @unless($promotion->isProductLaunch())<span><i class="bx bx-map-pin"></i>{{ $promotion->fulfillmentSummary() }}</span>@endunless @unless($promotion->is_active)<span class="is-muted"><i class="bx bx-pause-circle"></i>Pausada</span>@endunless</div>
                        </div>
                        <footer>
                ...
                <div class="promotion-modal__body">
                    ...
                    @elseif($wizardStep === 2)
                        @if($presentationType === 'new')
                            <section class="promotion-form-section">
                                ...
                                <section class="promotion-launch-offer">
                                    <label class="promotion-launch-offer__toggle"><input type="checkbox" wire:model.live="launchOfferEnabled"><span><i class="bx bx-purchase-tag-alt"></i><strong>Añadir oferta de lanzamiento</strong><small>El beneficio se aplica automáticamente al agregar la cantidad necesaria.</small></span></label>
                                    @if($launchOfferEnabled)
                                        <div class="promotion-form-grid is-three">
                                            <label><span>Compra <b>*</b></span><input type="number" wire:model="buyQuantity" min="1" max="99"><small>Unidades a precio normal.</small>@error('buyQuantity')<small class="field-error">{{ $message }}</small>@enderror</label>
                                            <label><span>Recibe <b>*</b></span><input type="number" wire:model="rewardQuantity" min="1" max="99"><small>Unidades beneficiadas.</small>@error('rewardQuantity')<small class="field-error">{{ $message }}</small>@enderror</label>
                                            <label><span>Descuento de recompensa <b>*</b></span><div class="promotion-input-suffix"><input type="number" wire:model="rewardDiscountPercentage" min="1" max="100"><span>%</span></div>@error('rewardDiscountPercentage')<small class="field-error">{{ $message }}</small>@enderror</label>
                                            <label><span>Máximo por pedido</span><input type="number" wire:model="maxApplicationsPerOrder" min="1" max="99" placeholder="Sin límite"><small>Vacío permite repetir el beneficio.</small>@error('maxApplicationsPerOrder')<small class="field-error">{{ $message }}</small>@enderror</label>
                                            <div class="promotion-rule-summary"><i class="bx bx-check-shield"></i><span><strong>Compra {{ $buyQuantity ?: 1 }} y recibe {{ $rewardQuantity ?: 1 }} al {{ $rewardDiscountPercentage ?: 50 }}%</strong><small>Solo descuenta el precio base; ingredientes y agregables conservan su precio.</small></span></div>
                                        </div>
                                    @endif
                                </section>
                            </section>
                    ...
                    @elseif($wizardStep === 3)
                        <section class="promotion-form-section">
                            ...
                            <label class="promotion-upload" for="promotion-campaign-image"><span>{{ $presentationType === 'new' ? 'Imagen alternativa' : 'Imagen horizontal del banner' }}</span><input id="promotion-campaign-image" type="file" wire:model.live="image" accept="image/jpeg,image/png,image/webp"><small>JPG, PNG o WEBP, máximo 4 MB. Recomendado: 1200 × 400 px.</small>@error('image')<small class="field-error">{{ $message }}</small>@enderror</label>
                            <div class="promotion-upload-preview" aria-live="polite" wire:loading.remove wire:target="image">
                                @if($image)
                                ...
                                @else
                                    <div><i class="bx bx-image-add"></i><strong>La imagen aparecerá aquí</strong><small>Así se mostrará la proporción horizontal de la campaña.</small></div>
                                @endif
                            </div>
                        ...
                        @if($presentationType !== 'new' || $launchOfferEnabled)
