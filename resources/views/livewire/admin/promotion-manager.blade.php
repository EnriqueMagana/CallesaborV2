                ...
                <div class="promotion-modal__body">
                    ...
                    @elseif($wizardStep === 2)
                        ...
                        @if($pricingMechanic !== 'fixed_price')
                            <section class="promotion-form-section">
                                @if(in_array($pricingMechanic,['two_for_one','second_half','custom_quantity'],true))
                                    <header><span><i class="bx bx-group"></i></span><div><h3>Grupo de productos elegibles</h3><p>El cliente podrá combinar cualquiera de estos artículos. En cada pareja, el beneficio se aplicará al de menor precio.</p></div></header>
                                    <fieldset class="promotion-products promotion-products--primary"><legend>Selecciona uno o varios productos <b>*</b><small>También puedes incluir un solo producto si deseas conservar la mecánica tradicional.</small></legend><div>@foreach($this->products as $product)<label><input type="checkbox" wire:model.live="eligibleProductIds" value="{{ $product->id }}"><span>@if($product->image)<img src="{{ Storage::url($product->image) }}" alt="" width="46" height="46">@else<i class="bx bx-dish"></i>@endif<strong>{{ $product->name }}</strong><small>{{ $product->category?->name ?: 'Sin categoría' }} · ${{ number_format($product->price, 2) }}</small></span></label>@endforeach</div>@error('eligibleProductIds')<small class="field-error">{{ $message }}</small>@enderror @error('eligibleProductIds.*')<small class="field-error">{{ $message }}</small>@enderror</fieldset>
                                @else
                    ...
                    @else
                        @php
                            ...
                            $includedProducts = $pricingMechanic === 'fixed_price'
                                ...
                                : (in_array($pricingMechanic,['two_for_one','second_half','custom_quantity'],true)
                                    ? collect($eligibleProductIds)->map(fn($id)=>$this->products->firstWhere('id',(int)$id)?->name)->filter()->unique()->values()
                                    ...
                                    <div class="promotion-device-frame__screen">
                                        ...
                                        @else
                                            <article class="promotion-preview-banner" @if($reviewImage) style="background-image:linear-gradient(90deg,rgba(12,25,17,.94) 0%,rgba(12,25,17,.68) 48%,rgba(12,25,17,.08) 100%),url('{{ $reviewImage }}')" @endif>
                                                <div><span class="promotion-preview-banner__badge"><i class="bx {{ $presentationType === 'discount' ? 'bx-purchase-tag' : 'bx-gift' }}"></i>{{ $mechanicLabel }}</span><h4>{{ $name ?: 'Nombre de la campaña' }}</h4><p>{{ $shortDescription ?: 'El resumen de la campaña aparecerá aquí.' }}</p>@if(in_array($pricingMechanic,['two_for_one','second_half','custom_quantity'],true) && $includedProducts->isNotEmpty())<small><i class="bx bx-group"></i>Combina {{ $includedProducts->join(', ') }}</small>@elseif($pricingMechanic !== 'fixed_price')<small><i class="bx bx-dish"></i>{{ $reviewProduct?->name }}</small>@elseif($includedProducts->isNotEmpty())<small><i class="bx bx-bowl-hot"></i>Incluye {{ $includedProducts->join(', ') }}</small>@endif<div class="promotion-preview-banner__bottom"><b>{{ $scheduleLabel }}</b><strong><small>{{ in_array($pricingMechanic,['fixed_price','fixed_product_price'],true) ? 'PRECIO PROMO' : 'BENEFICIO' }}</small>{{ in_array($pricingMechanic,['two_for_one','second_half','custom_quantity'],true) ? $mechanicLabel : '$'.number_format((float) $price, 2) }}</strong></div></div>
                                            </article>
