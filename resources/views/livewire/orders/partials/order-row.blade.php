...
<tr wire:key="order-row-{{ $order->id }}">
    ...
    <td><strong class="orders-cell-title">{{ $order->seller?->name ?? 'Sin asignar' }}</strong><small class="orders-muted">{{ $order->cashRegister?->name ?? 'Sin caja' }}</small></td>
    <td><strong class="orders-cell-title">{{ $order->created_at->format('d M Y') }}</strong><small class="orders-muted">{{ $order->created_at->format('H:i') }} h</small></td>
