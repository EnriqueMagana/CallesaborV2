<main class="kiosk-shell kiosk-tracking-shell">
    ...
    <header class="kiosk-header">
        <div class="kiosk-brand"><x-business.brand-mark :settings="$businessSettings" class="kiosk-brand-mark" /><span><strong>{{ $businessSettings?->business_name ?? config('app.name') }}</strong><small>Seguimiento de pedido</small></span></div>
        <span class="kiosk-live-badge"><i class="bx bx-pointer"></i> Actualización manual</span>
    </header>
    ...
    <section class="kiosk-tracking">
        ...
        @if($order->status !== 'cancelada')
            <div class="kiosk-status-timeline {{ $isDelivery ? 'is-delivery' : '' }}" aria-label="Estado del pedido">
                @php
                    $timeline = $isDelivery
                        ? [1 => ['bx-receipt','Recibido'], 2 => ['bx-bowl-hot','Preparando'], 3 => ['bx-check','Listo'], 4 => ['bx-cycling','En camino'], 5 => ['bx-home-heart','Entregado']]
                        : [1 => ['bx-receipt','Recibido'], 2 => ['bx-bowl-hot','Preparando'], 3 => ['bx-check','Listo']];
                @endphp
                @foreach($timeline as $index => $state)
                    <div class="kiosk-status-step {{ $current >= $index ? 'is-complete' : '' }} {{ $current === $index ? 'is-current' : '' }}"><span><i class="bx {{ $state[0] }}"></i></span><strong>{{ $state[1] }}</strong></div>
                @endforeach
            </div>
        @endif
        ...
        @if($isDelivery)
            <section class="kiosk-tracking-delivery-card">
                ...
                <div><small>Dirección de entrega</small><strong>{{ $order->customer_address }}</strong>@if($order->customer_references)<p>{{ $order->customer_references }}</p>@endif</div>
            </section>
        @endif

        <div class="kiosk-tracking-card">
            <div class="kiosk-section-title"><span><i class="bx bx-shopping-bag"></i></span><div><h2>Tu pedido</h2><p>{{ match($order->fulfillment) { 'dine_in' => 'Comer aquí', 'delivery' => 'Para domicilio', default => 'Para llevar' } }} · {{ $order->created_at->format('H:i') }}</p></div></div>
            <div class="kiosk-summary-lines">
                @foreach($order->items->where('is_cancelled', false) as $item)
                    <div class="kiosk-tracking-line"><div><strong>{{ $item->quantity }}× {{ $item->product_name }}</strong>@if($item->addons->isNotEmpty() || $item->ingredients->isNotEmpty())<small>{{ $item->addons->pluck('addon_name')->merge($item->ingredients->map(fn($ingredient) => $ingredient->ingredient_name.' ×'.$ingredient->quantity))->implode(' · ') }}</small>@endif</div><b>${{ number_format($item->subtotal, 2) }}</b></div>
                @endforeach
            </div>
            <div class="kiosk-cart-total"><span>Total</span><strong>${{ number_format($order->total, 2) }}</strong></div>
        </div>

        <div class="kiosk-manual-refresh">
            <button type="button" class="kiosk-primary-button" wire:click="refreshStatus" wire:loading.attr="disabled" wire:target="refreshStatus">
            ...
            <p class="kiosk-refresh-note" aria-live="polite"><i class="bx bx-info-circle"></i> Esta pantalla no se actualiza sola. Presiona el botón cuando quieras consultar cambios.@if($lastCheckedAt) Última consulta: {{ $lastCheckedAt }}.@endif</p>
        </div>
    </section>
</main>
