                        ...
                        <section class="customer-recent-orders" aria-labelledby="customer-orders-title">
                            ...
                            @forelse($selectedCustomer->orders as $order)
                                <div class="customer-recent-order">
                                    ...
                                    <span><strong>Pedido {{ $order->display_folio }}</strong><small>{{ $order->type_label }} · {{ \App\Support\BusinessTime::inTimezone($order->created_at)->translatedFormat('d M Y, g:i A') }}</small></span>
