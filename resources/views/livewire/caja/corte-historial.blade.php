    ...
    <section class="cash-history-list app-card">
        ...
        @forelse($this->cuts as $cut)
            ...
            <article class="cash-history-row">
                ...
                <div class="cash-history-row__main"><div class="cash-history-row__title"><strong>{{ $cut->folio }}</strong><span class="app-status app-status--neutral">{{ $cut->cashRegister->name }}</span>@if($diff===0.0)<span class="app-status app-status--success">Cuadrado</span>@elseif($diff>0)<span class="app-status app-status--info">Sobrante ${{ number_format(abs($diff),2) }}</span>@else<span class="app-status app-status--danger">Faltante ${{ number_format(abs($diff),2) }}</span>@endif</div><p>{{ \App\Support\BusinessTime::format($cut->cashRegister->opened_at) }} – {{ \App\Support\BusinessTime::format($cut->generated_at) }} · Cerró <strong>{{ $cut->generator?->name ?? 'Usuario no disponible' }}</strong></p></div>
