...
{
    // ── UI State ──
    public string $tab         = 'disponibles'; // disponibles | mis_mesas | todas
    ...
    {
        if (!$this->detailMesaId) return null;
    ...
    public function updatedSearch(): void   { unset($this->mesas); }
    public function updatedAreaFilter(): void { unset($this->mesas); }
    public function updatedStatusFilter(): void { unset($this->mesas); }
    ...
    {
        $mesa = Mesa::find($this->assignMesaId);
        if (!$mesa || $mesa->status !== 'disponible') {
    ...
    private function releaseMesaAndUngroup(Mesa $mesa): void
    {
        ...
        if ($groupId) {
            ...
            if ($remaining <= 1) {
                ...
                \App\Models\MesaGroup::destroy($groupId);
            }
        }
    }
    ...
    public function confirmUngroup(): void
    {
        ...
        if (!$mesa || !$mesa->mesa_group_id) return;
